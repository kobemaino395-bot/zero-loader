...
→ ModuleStomp (allowlist + synthetic RUNTIME_FUNCTION via RtlAddFunctionTable)
  ...
  → PhantomDllHollow (NTFS transaction + XOR-encrypted .text write)
  → NtAllocateVirtualMemory (4-tier fallback)
...
- **Stomper.c** — Three components: (1) `PickSacrificialDll` — picks a target from the 4-entry per-build allowlist (`xpsservices.dll`, `mfreadwrite.dll`, `dbgcore.dll`, `mfsensorgroup.dll`) with RDTSC-seeded Fisher-Yates rotation. Replaces the previous "scan all of System32" logic so we never select a Defender-reputation-flagged DLL (the prior `msftedit.dll` / `aadauthhelper.dll` / `amd_comgr.dll` choices were on Elastic/MDE public stomp-target rules). All four allowlist DLLs are signed Microsoft multimedia / debug / printing modules with low EDR scrutiny. (2) `ModuleStomp` — LoadLibrary the chosen sacrificial DLL + overwrite .text section with shellcode, reserve 4 DWORD-aligned bytes past the shellcode for a minimum UNWIND_INFO (leaf function, no prologue) and call `RtlAddFunctionTable` with a single `RUNTIME_FUNCTION` covering `[shellcode_start, shellcode_end)`. This silences Elastic 8.11+ kernel ETW callstack validation, which flags stomped regions with no matching `.pdata` entry. Falls back to plain stomp if `.text` lacks 4 spare bytes or `RtlAddFunctionTable` can't be resolved. (3) `BuildSyntheticStack` (opt-in via `ENABLE_SYNTHETIC_STACK`) — allocates 1 MB from the process heap and writes three fake return addresses (`NtWaitForSingleObject + 0x20`, `RtlUserThreadStart + 0x20`, `BaseThreadInitThunk + 0x20`) near the top; used by `SetSpoofStack` to swap RSP before shellcode executes, so kernel call-stack walkers see a plausible thread-start chain. Disabled by default because the 1 MB allocation and borrowed `.pdata` unwind coverage are themselves heuristic signals that require per-environment validation.
- **Phantom.c** — `PhantomDllHollow` opens a sacrificial DLL (via `PickSacrificialDll`) in an NTFS transaction (`CreateFileTransactedA`), **XOR-encrypts the shellcode in place against `INIT_PLACEMENT_XOR_KEY` (a per-build 16-byte key in Payload.h) before writing** to .text raw offset, creates SEC_IMAGE section (`NtCreateSection`), rolls back transaction (on-disk file unchanged), maps section (`NtMapViewOfSection`), then in the mapped memory: flips RX → RW, XOR-decrypts in place, flips RW → RX/RWX. Writing encrypted bytes defeats Defender's MpFilter transaction-aware scanner (2022+), which previously fired `Behavior:Win32/Meterpreter.gen` / `Trojan:Win32/SuspGolang.AM` alerts on the in-flight transactional view. The plaintext shellcode is restored on the heap buffer after the write so subsequent fallback paths see plaintext.
...
- **`INIT_PLACEMENT_XOR_KEY`** (16 random bytes) — used by Phantom and GhostlyHollow to encrypt shellcode in the file-write step, defeating Defender's MpFilter content scan
